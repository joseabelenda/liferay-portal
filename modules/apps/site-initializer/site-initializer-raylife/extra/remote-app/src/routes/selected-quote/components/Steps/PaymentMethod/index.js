...
const PaymentMethod = ({product, selectedQuote}) => {
	...
	const onPayNow = async () => {
		...
		const response = await fetch(
			`http://localhost:3333/api/commerce/${selectedQuote.orderId}`
		);

		const {
			data: {groupId, uuid_: uuid},
		} = await response.json();

		await updateOrderPaymentMethod(selectedQuote.orderId);

		const nextStep = `${origin}${LiferayService.getLiferaySiteName()}/congrats`;

		console.error(
			`${origin}/o/commerce-payment?groupId=${groupId}&uuid=${uuid}&nextStep=${nextStep}`
		);

		if (confirm('Do you want to redirect ? ')) {
			window.location.href = `${origin}/o/commerce-payment?groupId=${groupId}&uuid=${uuid}&nextStep=${nextStep}`;
		}
	};
